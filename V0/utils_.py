...
class Trie:
    ...
    def insert(self,w):
        ...
        current = self.root
        for c in w:
            current = current.children[c]       # key不存在时自动生成val为TrieNode的键值对并返回
        current.is_w = True                     # 单词结束的标志位
    ...
    def search(self,w):
        ...
        return 1 if current.is_w else 0

    def get_lexicon(self,sentence):
        '''
        匹配找到前缀树中所有出现在句子中的词
        返回样式为 [[start, end, text]]
        '''
        result = []
        for i in range(len(sentence)):
            ...
            for j in range(i, len(sentence)):
                ...
                if current.is_w: result.append([i,j,sentence[i:j+1]])
        return result
