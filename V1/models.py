...
class Lattice_Transformer_SeqLabel(nn.Module):
    ...
    def forward(self, lattice, bigrams, seq_len, lex_num, pos_s, pos_e,
        ...
        '''
        raw_embed_char: lattice embed, bigrams embed, bert embed
            拼的是最后一维，相当于把三种词向量拼接起来，50+50+768
            bert embed只把所有字符用bert编码
        raw_embed: lattice embed, 包含字符和词
        第二维都是char+lexicon的长度
        把嵌入维度统一投影到160维
        '''
        ...
        # print('447 models', list(embed_char.shape), list(embed_lex.shape))
